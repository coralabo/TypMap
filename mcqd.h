...
class Maxclique {
    ...
    Vertex& end() const { return v[sz - 1]; };
  	...
  	public:
		...
		void expand(Vertices,const int smax);//
		void expand_dyn(Vertices,const int smax);//
		void _mcq(int*&, int&,int&, bool);
	...
	void mcq(int* &maxclique, int &sz,int &smax) { _mcq(maxclique, sz, smax,false); }//
	void mcqdyn(int* &maxclique, int &sz,int &smax) { _mcq(maxclique, sz,smax, true); }//
	~Maxclique() {
		if (C) delete [] C;
		if (S) delete [] S;
		V.dispose();
